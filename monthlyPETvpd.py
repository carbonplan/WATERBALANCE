...
@function
def monthlyPET(radiation=None,tmax=None,tmin=None,wind=None,Lat=None,Z=None,albedo=None,vpd=None,*args,**kwargs):
...
# monthlyPETvpd.m:86
            Ra=multiply(multiply(multiply(dot(24,60) / pi,GSC),dr),(multiply(multiply(omegas,sin(phi)),sin(delta)) + multiply(multiply(cos(phi),cos(delta)),sin(omegas))))
# monthlyPETvpd.m:87
...
# monthlyPETvpd.m:107
        radfract[f]=1
# monthlyPETvpd.m:107
        longw=multiply(dot(dot(4.903e-09,((tmax(arange(),doy) + 273.15) ** 4 + (tmin(arange(),doy) + 273.15) ** 4)) / 2.0,(0.34 - dot(0.14,sqrt(ea(arange(),doy))))),(dot(1.35,radfract) - 0.35))
# monthlyPETvpd.m:108
        #[nmean(longw(:))]
# netrad
        netrad=dot(real(multiply(radiation(arange(),doy),(1 - albedo)) - longw),daysinmonth(doy))
# monthlyPETvpd.m:111
        #nmean(netrad(:))
#soil heat flux
...
# monthlyPETvpd.m:117
        TERM2_NUMERATOR=dot(multiply(multiply(multiply(GAM(arange(),doy),wind(arange(),doy)),vpd(arange(),doy)),900.0) / (273.15 + tmean(arange(),doy)),daysinmonth(doy))
# monthlyPETvpd.m:118
        DENOMINATOR=DEL(arange(),doy) + multiply(GAM(arange(),doy),(1 + multiply(0.34,wind(arange(),doy))))
# monthlyPETvpd.m:119
